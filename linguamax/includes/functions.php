...
function getStreak($userId) {
    static $streakCache = [];
    if (isset($streakCache[$userId])) return $streakCache[$userId];
    $db = getDB();
    $stmt = $db->prepare("SELECT * FROM user_streaks WHERE user_id = ?");
    $stmt->execute([$userId]);
    $res = $stmt->fetch() ?: ['current_streak' => 0, 'longest_streak' => 0, 'last_activity_date' => null];
    $streakCache[$userId] = $res;
    return $res;
}
...
function getLeaderboard($limit = 10) {
    $db = getDB();
    $stmt = $db->prepare("
        SELECT u.id, u.code, u.fname, u.lname, u.nickname, u.xp, u.coins, u.level, u.avatar_color, u.character_id,
