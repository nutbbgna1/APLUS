...
switch ($action) {
    // ── SETTINGS ──────────────────────────────────────────────────
    case 'toggle_desktop_block':
        $block = ($input['block_desktop'] ?? $_POST['block_desktop'] ?? '0') === '1';
        ...
        $globalSettings['block_desktop'] = $block;
        file_put_contents($settingsFile, json_encode($globalSettings, JSON_PRETTY_PRINT));
    ...
    case 'add_student':
        ...
        try {
            ...
            $stmt->execute([
                ...
                $lname, 
                $nickname, 
                ['#10B981','#3B82F6','#F59E0B','#EF4444','#8CB3FF'][rand(0,4)]
            ]);
            jsonResponse(['success' => true]);
    ...
    case 'save_exam_permissions':
        $userId = intval($input['user_id'] ?? 0);
        $exams = $input['exams'] ?? [];
        if (!$userId) jsonResponse(['error' => 'Missing user ID'], 400);
        ...
        try {
            ...
            if (!empty($exams) && is_array($exams)) {
                ...
                foreach ($exams as $examId) {
                    $stmt->execute([$userId, intval($examId)]);
                }
            }
            $db->commit();
            jsonResponse(['success' => true]);
